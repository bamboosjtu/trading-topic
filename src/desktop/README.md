...
- 外部响应结构、错误码、限流和年度异常缺口严格校验；回测数据与实验原子提交，实盘行情使用独立缓存并按页面所需区间补齐；
- 七项回测指标、前复权真实 OHLCV K 线、收益率/最大回撤曲线、逐笔记录、数据来源和截止日；
- 持仓明细只读页：当前持仓、成本、估值、累计收益、组合/标的区间表现与详情抽屉；
- 交易流水事实写入页：资金转入/转出、买入、卖出、现金分红和国债逆回购录入；领域影响预览；追加式修正/冲正；筛选、分页、详情与 XLSX 导出；
- 收益日历只读页：月度、累计、年内收益，市场价格/分红/交易影响/逆回购四项日度归因、标的贡献与流水跳转；
- `loading / empty / stale / partial / error` 明确区分；缺行情不以成本价冒充市值；
- 实盘保持单账户；不提供账户切换或券商流水导入，交易流水页是唯一手工事实写入口；
