...
npm test
npm run build
# 发布前受控联网验证目录、行情、股票/ETF 公司行动、停复牌主备源、历史停牌回归向量和来源落库；
...
- 沪深京全 A 股与境内交易所 ETF 代码/名称检索（完整分页目录、本地完整快照）；历史回测与实盘录入均支持 A 股和境内 ETF，单次回测最多 10 个标的并允许混合比较；
...
- 股票公司行动合并东方财富与同花顺 F10 已实施记录，ETF 分红合并新浪累计因子与东方财富基金 F10；同日语义冲突阻断，只有完整命中正式公告评审向量的差异化分红才按二级市场普通股权益校准，单源漏项显式进入 provenance；配股只报告、不参与；
...
- 实盘行情只在双源均明确返回空且独立交易日历确认整个区间休市时记录空覆盖；任一源失败时保持待重试，避免把临时故障永久写成“无交易数据”；发布 smoke 覆盖目录、行情、公司行动、停复牌八项能力及已知历史缺口回归向量并保留结果；
- 七项回测指标、前复权真实 OHLCV K 线、收益率/最大回撤曲线、逐笔记录、数据来源和截止日；
- 持仓明细只读页：事实截止日与正式估值截止日分离；估值截止日后的买入、卖出或分红仍进入事实类累计指标，但持仓市值、投资总收益、XIRR、组合/标的区间表现统一为 `null / partial`，不把后到现金投影到更早终值；
- 交易流水事实写入页：买入、卖出和单日期现金分红录入；买卖休市日校验；分红并再投入的合并影响预览与原子写入；关联修正执行同标的、同资产类型、日期顺序和一分红一买入组级校验；筛选、分页、业务化关联详情与 XLSX 导出；
...
- 设置页展示 Schema、固定费用/口径、八项数据源实时健康、A 股与 ETF 目录实际来源、兜底原因和年度交易日历状态，并提供 JSON 备份恢复、脱敏日志导出；2011—2026 最大 15 年快捷回测区间均有官方日历，当前年度日历缺失时发布构建失败；
- 备份恢复在覆盖事务前完整校验账本引用图与归约、事实类型专属字段、唯一修正版本、回测试验、行情快照与覆盖、目录、公司行动及工作区，非法备份不触碰现有数据；
- 仅支持当前 Schema 1 和当前备份契约；不迁移、不兼容旧数据库、旧备份或缺失资产类型/provenance 的旧事实；
- 明确不连接券商、不执行交易。
