...
测试与产品源码同域，运行时不读取或执行 `labs/`、`research/`。各层由 `vitest.config.mts` 显式编组：
...
- smoke 层允许联网，但必须显式触发并保留 `artifacts/market-data-smoke.json`；当前门禁覆盖沪深京与 ETF 目录、腾讯/新浪行情、股票/ETF 公司行动、`600900` 差异化分红官方校准向量、停复牌主备源、已知历史停牌向量、15 年严格行情完整性和来源落库；
