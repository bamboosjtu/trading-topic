...
export interface DecodedSinaBar {
  date: unknown;
  open: unknown;
  high: unknown;
  low: unknown;
...
// Sina publishes the full daily history as a compact KLC payload. This fixed
// decoder is executed in an isolated VM; the remote payload is passed only as
// JSON data and is never evaluated as source code.
const SINA_KLC_DECODER = String.raw`
function d(t) {
...
export function decodeSinaKlc(encoded: string): DecodedSinaBar[] {
  const context: Record<string, unknown> = {};
  runInNewContext(SINA_KLC_DECODER, context, {
    timeout: 5_000,
  });
  const decoded = runInNewContext(
    `d(${JSON.stringify(encoded)})`,
    context,
    { timeout: 5_000 },
  ) as unknown;
  if (!Array.isArray(decoded)) {
    throw new Error("新浪全量日线解码结果不是数组");
  }
  return decoded as DecodedSinaBar[];
}
