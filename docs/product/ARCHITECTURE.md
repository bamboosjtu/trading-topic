...
- 东方财富公司行动先做完整行去重，再依次使用源方案 ID、报告期、方案公告日识别同一方案；同一方案的多个版本只保留最终已实施记录。只有方案身份明确不同但除权日相同的记录才累加。转增优先读取真实响应的 `IT_RATIO`，`BONUS_IT_RATIO` 仅作缺字段时的送转合计回退。
- 回测行情或公司行动请求失败、响应结构变化、无数据或存在尚未实现的公司行动（如配股）时直接报错，不生成虚构行情，也不静默切换来源。现金分红、送股和转增属于 R1 已支持事件。
- 前复权 K 线是独立的非证据型浏览数据。失败时实验仍可完成，但结果携带明确的 `error` 状态与原因；严禁用不复权收盘价伪造 OHLC。
...
- `useBacktestWorkspace()` 是 Renderer 唯一的工作区写入口；页面事件只更新 React 状态。活动实验读取失败时显示错误、清除 `activeExperimentId` 并回到空白运行态，不自动选择其他实验。
...
| 范围     | 验证                                                                                                                  |
| -------- | --------------------------------------------------------------------------------------------------------------------- |
| 领域单测 | 日期顺延、零碎股、费用 0、分红、送转、XIRR、最大回撤与最长回撤周期、账本冲正与逆回购                              |
...
| 持久化   | 每次运行新增不可变实验、结果归属与删除、工作区活动实验和股票目录重开恢复、四标的完整冷启动链路、schema v5 备份恢复     |
