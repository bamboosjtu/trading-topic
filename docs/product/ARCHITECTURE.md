...
| 数据 | 产品主源 | 口径 |
| --- | --- | --- |
| 全 A 股代码与名称 | 东方财富 `push2 clist` | 与 `stock_info_a_code_name()` 等价的代码/简称目录 |
| A 股日线 | 腾讯财经 `newfqkline` | 不复权收盘价 |
| 已实施公司行动 | 东方财富 `RPT_SHAREBONUS_DET` | 税前每股现金分红；每 10 股送股/转增比例 |

代码表由产品自有 Node.js 适配器直接请求底层数据接口，不执行 Python 或
AkShare。成功快照写入 SQLite，七天内直接使用；刷新失败时回退到上次成功
快照。每次回测记录来源、获取时间、实际数据截止日、复权口径和口径版本。
...
- `backtest_runs.strategy_key`：由标的、区间、金额、买入日、分红口径和计算口径组成的唯一键；同参数采用 upsert；
- `backtest_runs.batch_id`：同一次多标的回测的批次标识，用于重启后恢复图表；
- `backtest_workspace`：上次参数、图表选项和最后结果批次；
...
回测详情只从 `backtest_runs` 的结果快照转换，不重新联网或二次模拟。XLSX 在
Electron 主进程生成：汇总 sheet 后跟每条“标的 + 参数”的明细 sheet，渲染层
只通过受限 IPC 发起保存。
...
| 持久化 | 同参数 upsert、不同参数新增、工作区与股票目录重开恢复、schema 1 备份兼容 |
| 导出 | XLSX 汇总字段与每个“标的 + 参数”明细 sheet 名称、内容 |
| 界面冒烟 | 1920×1080 下全市场搜索、最多 10 标的、图表切换、对比翻页、明细筛选与翻页；其余入口显示骨架屏 |
