...
代码表由产品自有 Node.js 适配器直接请求三家交易所公开接口，不执行 Python 或
AkShare。三地列表全部成功并通过全量完整性校验后才写入 SQLite；七天内直接
使用完整快照，刷新失败时只回退到上次成功的完整快照。少量示例标的或旧的
不完整缓存不得作为全 A 股目录返回。每次回测记录来源、获取时间、实际数据
截止日、复权口径和口径版本。
...
- `backtest_experiments`：每次点击生成的不可变实验，保存请求、运行时间、共同
  数据截止日、口径版本和状态；
- `backtest_results`：实验内每个标的的结果快照，通过 `experiment_id` 归属实验；
- `backtest_results.strategy_key`：标的与参数的非唯一识别索引，只用于查询和
  诊断，不用于 upsert 或覆盖；
- `backtest_workspace`：上次表单与图表选项，以及当前查看的
  `active_experiment_id`；
- `stock_universe`：全 A 股代码与名称的本地缓存快照；
- `market_prices`：产品自行获取的行情快照；
- `corporate_actions`：产品自行获取的公司行动；
- `settings`：固定口径与本地配置；
- `app_logs`：脱敏运行日志。

写操作完成后立即持久化。JSON 备份同时包含股票目录与回测工作区。恢复先校验
应用标识和 schema 版本，再生成恢复前安全备份，最后在事务中覆盖业务数据；
校验或事务失败时不破坏当前数据库。

每次运行先创建一个实验编号，再以同一请求和共同数据截止时间计算全部标的；
实验与结果在一个事务中插入，禁止原地更新。删除实验时同时删除其全部结果，
并清除工作区可能存在的活动引用。

回测详情只从 `backtest_results` 的结果快照转换，不重新联网或二次模拟。XLSX
在 Electron 主进程按实验生成：汇总 sheet 后跟该实验每个标的的明细 sheet，
渲染层只通过受限 IPC 发起保存。

当前没有用户数据且不要求向后兼容，因此 schema v3 直接删除废弃的
`backtest_runs`/批次模型，不保留双写、迁移适配或旧备份恢复分支。JSON 恢复
只接受当前 schema。
...
“系统 / 设置”。历史回测内部再分“运行回测 / 历史结果”：前者只展示当前
实验，后者展示不可变实验列表及查看、复制参数、删除操作。历史回测是当前唯一完成态页面，其余四个入口统一渲染
