...
- 持仓与收益日历通过 `PositionsOverview` 和 `IncomeCalendarView` 只读消费领域结果；交易流水页通过 `LedgerQueryResult` 查询事实，并通过 `previewLedger / addLedger / addDividendReinvestment / correctLedger / reverseLedger` 五类受限命令写入投资事实。`ledgerReducer.ts` 是唯一归约实现，统一同日顺序、历史重述、显式截止日、持仓成本、买入支出、卖出净收入、分红和已实现收益；`ledgerCommands.ts` 只负责命令校验与影响预览；`ledgerQuery.ts`、`positionsView.ts`、`dailyAttribution.ts`、`incomeCalendar.ts` 分别负责流水查询、持仓视图、日度三项归因和收益日历汇总，不得复制金融公式。Renderer 只负责表单、筛选、格式化和布局。
- 普通录入只接受买入、卖出和现金分红；`adjustment` 不能从普通表单创建。分红并再投入以一个 SQLite 事务写入两条共享关联编号的独立事实。追加修正以一个事务写入“撤销原影响”的 adjustment 和修正后事实；冲正只追加 adjustment。原记录不覆盖、不删除，重复冲正或会造成历史卖空的变更会在落库前拒绝。
...
- 投资收益现金流只有三类：买入及费用为负，卖出净收入与现金分红为正，期末持仓市值为期末正现金流。组合总收益恒等于“当前持仓市值 + 累计卖出净收入 + 累计分红 − 累计买入支出”，不得依赖账户余额或额外资金事实。
...
- `latestCompletedTradingDate()` 是正式收盘日唯一边界：交易日 15:10 前取上一交易日，15:10 后仅在日线完整时取当日，周末/节假日取上一交易日，历史月份取月内最后有效交易日。只有交易事实而无当日正式收盘时进入 `partial`，不得以前收冒充。
...
| 适配器   | 合法空响应、源错误码、结构损坏、占位符数值、真实配股 fixture、腾讯缺字段与年度异常缺口、正式收盘日、新浪整段兜底与跨源冲突                              |
...
| 持久化   | 每次运行新增不可变实验、结果归属与删除、实验缓存原子回滚、回测/实盘行情隔离、工作区活动实验和股票目录重开恢复、四标的完整冷启动链路、schema v8 备份恢复、组合事实原子写入 |
| 导出     | XLSX 汇总字段与每个“标的 + 参数”明细 sheet 名称、内容                                                                 |
| 实盘领域 | 唯一同日排序、显式截止日、未来日期拒绝、实际股数/份额、买卖分红现金流、影响预览、历史重述、分红再投入原子追加、已清仓收益、三项日度收益恒等式、月度和累计指标 |
